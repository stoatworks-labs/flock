...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/flock/` |
| Linux | `$XDG_STATE_HOME/flock/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\flock\logs\` |

`FLOCK_LOG_DIR` overrides it. The path is printed on the first line of every
...
`flock.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `FLOCK_LOG` (falling back to `RUST_LOG`), default `flock=info`.
...
`flock-crash-<timestamp>.json`; the path is printed on the way down. The
...
```bash
flock --collect-diagnostics
```

Writes `flock-diagnostics-<timestamp>.json` and prints its path. One file, so
