...
```sh
xattr -dr com.apple.quarantine ./flock
chmod +x ./flock
./flock --help
```
...
flock listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of flock**
...
domain. Flock needs this to serve the web UI and find BirdDog decoders by broadcast discovery on your LAN.

If you deny it (or the prompt appears behind another window and times out), discovery will come back empty and the UI won't be reachable from another machine
...
**Private**. If flock isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "flock" -Direction Inbound -Program "C:\Path\To\flock.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*flock*" | Remove-NetFirewallRule
```
...
```sh
codesign --force --deep --sign - "/Applications/flock.app"
```

Verify it took:

```sh
codesign -dv --verbose=4 "/Applications/flock.app"
spctl -a -vv "/Applications/flock.app"   # still reports "rejected" — ad-hoc is not notarization
```
